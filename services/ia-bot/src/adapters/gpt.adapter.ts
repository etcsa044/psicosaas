/**
 * GPT Adapter — abstracts the LLM integration.
 * Implements the Adapter pattern to allow swapping GPT for other LLMs.
 */
...
export class GPTAdapter implements ILLMAdapter {
    ...
    async generateResponse(systemPrompt: string, userMessage: string, context?: string): Promise<LLMResponse> {
        if (!this.apiKey) {
            ...
            return {
                content: `[DEV MODE] Received: "${userMessage.substring(0, 50)}..."`,
                tokensUsed: 0,
                model: 'dev-mock',
            };
        }
